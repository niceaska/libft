...
static int		ft_print_handler(char *str, long size, t_pr *vals,  va_list args)
{
	if (*str == '\0' || !str || size == -1)
		return ((int)size);
	while (*str && *str != '%')
	...
	if (*str == '%')
	{
		...
		if (!mods_checker(str + 1))
		{
			...
			while (check_multipy_mods(str))
				str++;
			return (ft_print_handler(str + 1, size, vals, args));
		}
	...
	clear_struct(&vals);
	return (ft_print_handler(str, size, vals, args));
}
...
int					ft_printf(const char *format, ...)
{
	...
	if (!(vals = init_val()))
		return (-1);
	size = ft_print_handler((char *)format, 0, vals, arg);
	size += vals->print;
	if (vals->index)
